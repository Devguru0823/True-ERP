...
import { DeliveryService } from '../js/delivery-service';
...
Template.employeescard.onCreated(function () {
    ...
    templateObject.changeEmployeeName = new ReactiveVar();
    ...
    templateObject.defaultcardcharts = new ReactiveVar();

    let defaultcardchartsData = [
        {index: 0, type: 'card', Position: 0, ChartWidth: "465", ChartHeight: '148', ChartName: 'all-crm-tasks', ChartGroup: 'Employee', Active: true},
        {index: 1, type: 'card', Position: 1, ChartWidth: "465", ChartHeight: '148', ChartName: 'today-crm-tasks', ChartGroup: 'Employee', Active: true},
        {index: 2, type: 'card', Position: 2, ChartWidth: "465", ChartHeight: '148', ChartName: 'upcoming-crm-tasks', ChartGroup: 'Employee', Active: true},
        {index: 3, type: 'card', Position: 3, ChartWidth: "465", ChartHeight: '148', ChartName: 'crm-projects', ChartGroup: 'Employee', Active: true},
    ]

    templateObject.defaultcardcharts.set(defaultcardchartsData);
        ...
        if(parseInt(data.fields.EmployeeID) == parseInt(employeeID)){
            if(data.fields.Active === true){
                dataList = [
                    ...
                    moment(data.fields.StartDate).format('DD/MM/YYYY') || "",
                    moment(data.fields.EndDate).format('DD/MM/YYYY') || "",
                    // data.fields.Active || '',
    ...
    let headerStructure = [
        ...
        { index: 4, label: 'Start Date', class: 'colStartDate', active: true, display: true, width: "120" },
        { index: 5, label: 'End Date', class: 'colEndDate', active: true, display: true, width: "120" },
        { index: 6, label: 'Status', class: 'colLRStatus', active: true, display: true, width: "120" },
    ];
    ...
    templateObject.getTransactionDataTableList = function(data) {
        var dataList = [
            ...
            data.SaleID || "",
            data.CustomerName || "",
            utilityService.modifynegativeCurrencyFormat(data.TotalAmount)|| 0.00,
            utilityService.modifynegativeCurrencyFormat(data.TotalTax)|| 0.00,
            utilityService.modifynegativeCurrencyFormat(data.TotalAmountInc)|| 0.00,
            utilityService.modifynegativeCurrencyFormat(data.Payment)|| 0.00,
            utilityService.modifynegativeCurrencyFormat(data.Balance)|| 0.00,
            data.SaleCustField1 || "",
            data.SaleCustField2 || "",
            data.EmployeeName || "",
            data.Comments || "",
    ...
    let transactionHeaderStructure = [
        ...
        { index: 8, label: 'Balance Outstanding', class: 'colBalance', active: true, display: true, width: "110" },
        ...
        { index: 11, label: 'Employee', class: 'colEmployee', active: true, display: true, width: "200" },
    ...
    templateObject.productTableHeaderRecords = new ReactiveVar([]);
    templateObject.getProductDataTableList = function(data) {
        var dataList = [
            data.fields.ID || "",
            data.fields.EmployeeName || "",
            data.fields.AbilityDesc || "",
            utilityService.modifynegativeCurrencyFormat(data.fields.PayRate) || 0.00,
            utilityService.modifynegativeCurrencyFormat(data.fields.Rate) || 0.00,
            data.fields.Active == true ? '' : 'In-Active',
        ];
        return dataList;
    }
    let productHeaderStructure = [
        { index: 0, label: 'ID', class: 'colProductID', active: false, display: true, width: "10" },
        { index: 1, label: 'Name', class: 'colServiceName', active: true, display: true, width: "200" },
        { index: 2, label: 'Description', class: 'colServiceDescription', active: true, display: true, width: "300" },
        { index: 3, label: 'Cost Price', class: 'colServiceCostPrice', active: true, display: true, width: "110" },
        { index: 4, label: 'Sale Price', class: 'colServiceSalesPrice', active: true, display: true, width: "110" },
        { index: 5, label: 'Status', class: 'colStatus', active: true, display: true, width: "120" },
    ];
    templateObject.productTableHeaderRecords.set(productHeaderStructure);
});
...
Template.employeescard.onRendered(function () {
    ...
    $(document).ready(function () {
        setTimeout(function () {
            ...
            $('#edtPayPeriod').editableSelect('add', 'Fortnightly');
            $('#edtPayPeriod').editableSelect('add', 'Twice Monthly');
            $('#edtPayPeriod').editableSelect('add', 'Four Weekly');
            ...
            $('#edtHolidayGroup').editableSelect()
                .on('click.editable-select', async function (e, li) {
                    ...
                    let offset = $search.offset();
                    if (e.pageX > offset.left + $search.width() - 8) { // X button 16px wide?
                        $('#groupTypeListModal').modal("toggle");
                    } else {
                        $('#groupTypeListModal').modal("toggle");
                    }
                });
        ...
        $(document).on("click", "#tblTitleList tbody tr", function (e) {
            ...
            let updatedValue = $(this).find(".colTitleName").text();
            if (prevValue !== updatedValue) {
                localStorage.setItem("isFormUpdated", true);
            }
            $('#editEmployeeTitle').val($(this).find(".colTitleName").text());
        ...
        $(document).on("click", "#tblgrouptypelist tbody tr", function (e) {
            $('#edtHolidayGroup').val($(this).find(".colgroupDescription").text());
            $('#groupTypeListModal').modal('toggle');
        });

        $(document).on("click", "#tblPayPeriods tbody tr", function (e) {
            $('#edtPayPeriod').val($(this).find('.colPayPeriodName').text());
        });
    ...
    $(document).on('shown.bs.tab', 'a[data-toggle="tab"]', async function (e) {
        let target = $(e.target).attr("data-target") // activated tab
        if (target === "#tab-delivery" || target === "#tab-vehicles") {
            console.log("target===============");
            $('#tblDriverVehiclesList_filter .form-control-sm').get(0).focus();
        } else if (target === "#tab-skills") {
            $('#tblDriverSkillsList_filter .form-control-sm').get(0).focus();
        } else if (target === "#tab-1") { // transaction tab
            await clearData("TInvoiceEx");
            templateObject.changeEmployeeName.set(false);
            setTimeout(() => {
                templateObject.changeEmployeeName.set(true);   
              },1000)
            setTimeout (()=>{
                $('#tblEmployeeTransactionList_filter .form-control-sm').get(0).focus();
            },3000)
...
Template.employeescard.onRendered(function () {
    ...
    $(document).on("click", "#vehicleFeaturesPop #tblvehiclefeatureslist tbody tr", function (event) {
        let value = $(event.currentTarget).find('.colVehicleFeatureName').text();
        $(`#${selectFeaturesID}`).val(value);
        $("#vehicleFeaturesPop").modal("hide");
    });

    $(document).on("click", "#paymentTypesListPopModal tbody tr", (event) => {        
        $("#basicprice").val($(event.currentTarget).find('.colBasicPrice').text())
        $("#basicduration").val($(event.currentTarget).find('.colBasicDuration').text())
        $("#basicdistance").val($(event.currentTarget).find('.colBasicDistance').text())
        $("#durationprice").val($(event.currentTarget).find('.colDurationPrice').text())
        $("#distanceprice").val($(event.currentTarget).find('.colDistancePrice').text())
        $("#cancelprice").val($(event.currentTarget).find('.colCancelPrice').text())
        $("#commissionpercent").val($(event.currentTarget).find('.colCommissionPercent').text())
        $("#commissionfixed").val($(event.currentTarget).find('.colCommissionFixed').text())
    })
...
Template.employeescard.events({
    ...
    'click .btnRefreshCrm': function () {
        ...
        crmService.getAllTasksList().then(function (data) {
            addVS1Data('TCRMEmployeeTaskList', JSON.stringify(data)).then(function (datareturn) {
                if (!isNaN(currentId.jobid)) {
                    window.open('/employeescard?jobid=' + currentId.jobid + '&transTab=crm', '_self');
                }
                if (!isNaN(currentId.id)) {
                    window.open('/employeescard?id=' + currentId.id + '&transTab=crm', '_self');
                }
                window.open('/employeescard?transTab=crm', '_self');
            }).catch(function (err) {
                if (!isNaN(currentId.jobid)) {
                    window.open('/employeescard?jobid=' + currentId.jobid + '&transTab=crm', '_self');
                }
    ...
    'click .btnRefreshProduct': function () {
        let currentId = FlowRouter.current().queryParams;
        $('.fullScreenSpin').css('display', 'inline-block');
        contactService.getAllInvoiceList().then(function (data) {
            addVS1Data('TInvoiceEx', JSON.stringify(data)).then(function (datareturn) {
                if (!isNaN(currentId.jobid)) {
                    window.open('/employeescard?jobid=' + currentId.jobid + '&transTab=Product', '_self');
                }
                if (!isNaN(currentId.id)) {
                    window.open('/employeescard?id=' + currentId.id + '&transTab=Product', '_self');
                }
                window.open('/employeescard?transTab=Product', '_self');
            }).catch(function (err) {
                if (!isNaN(currentId.jobid)) {
                    window.open('/employeescard?jobid=' + currentId.jobid + '&transTab=Product', '_self');
                }
    ...
    'click #btnNewNonInventoryProduct': function(event){
        $("#productListModal").modal("show");
        setTimeout(function(){
            $('#tblInventoryService_filter .form-control-sm').get(0).focus();
        },1000)
    },
    ...
    'click .btnSave': async (e, templateObject) => {
        ...
        setTimeout(async function () {
            ...
            let edtDashboardOptions = $('#employeeDashboard').val() || '';
            let utilityService = new UtilityService();
            ...
            if (!isNaN(currentId.id)) {
                ...
                objDetails = {
                    ...
                    fields: {
                        ...
                        LoginDefault: edtDashboardOptions, // tempcode until the fields are added in backend
    ...
    'click #btnSaveEmployeePayroll': async (event) => {
        ...
        if ($('div#employment').attr("class").indexOf("active") >= 0) activeTab = "employment";
        ...
        if (activeTab == "taxes") {
            ...
            let StartDate = $('#edtFirstPayDate').val();
            let Classification = $('#edtClassificationEmployee').val();
            let PayrollCalendar = $('#edtPayrollCalendar').val();
            let EmploymentGroup = $('#edtEmploymentGroup').val();
            let HolidayGroup = $('#edtHolidayGroup').val();
            let EarningRate = $('#edtEarningRate').val();

            let includeHolidaysInPayslips = $("#employee_includeHolidaysInPayslips_toggle").is(':checked') ? true : false;
            let authorisedToApproveLeave = $("#employee_authorisedToApproveLeave_toggle").is(':checked') ? true : false;
            let authorisedToApproveTimesheets = $("#employee_authorisedToApproveTimesheets_toggle").is(':checked') ? true : false;

            if(StartDate == "") {
                handleValidationError('Please select Start Date!', 'edtFirstPayDate');
                return false;
            }
            if(Classification == "") {
                handleValidationError('Please enter Classification!', 'edtClassificationEmployee');
                return false;
            }
            if(PayrollCalendar == "") {
                handleValidationError('Please select Payroll Calendar!', 'edtPayrollCalendar');
                return false;
            }
            if(EmploymentGroup == "") {
                handleValidationError('Please select Employment Group!', 'edtEmploymentGroup');
                return false;
            }
            if(HolidayGroup == "") {
                handleValidationError('Please select Holiday Group!', 'edtHolidayGroup');
                return false;
            }
            if(EarningRate == "") {
                handleValidationError('Please select Earning Rate!', 'edtEarningRate');
                return false;
            }
            LoadingOverlay.show();
    ...
    'click .new_attachment_btnAddEmployee': function (event) {
        $('#addEmployee-attachment-upload').trigger('click');
    },
    ...
    'change #addEmployee-attachment-upload': function (e) {
        let templateObj = Template.instance();
        let saveToTAttachment = false;
        let lineIDForAttachment = false;
        let uploadedFilesArray = templateObj.uploadedFiles.get();

        let myFiles = $('#addEmployee-attachment-upload')[0].files;
    ...
    'click .confirm-delete-attachment': function (event, ui) {
        ...
        $('#addEmployee-attachment-upload').val('');
    ...
    "click .edtVehicleFeatures": function (event) {
        selectFeaturesID = event.target.id || "";
        $("#vehicleFeaturesPop").modal("toggle");
        setTimeout(() => {
            $('#tblvehiclefeatureslist_filter .form-control-sm').get(0).focus();
        }, 1000)
    },
});
...
Template.employeescard.helpers({
    defaultcardcharts: () => {
        return Template.instance().defaultcardcharts.get()
    },
    ...
    contactID: function() {
        return Template.instance().records.get().company ||'';
    },
    changeEmployeeName: function() {
        return Template.instance().changeEmployeeName.get()
    },
    ...
    productTableHeaderRecords: () => {
        return Template.instance().productTableHeaderRecords.get();
    },
    productApiFunction:function() {
        let sideBarService = new SideBarService();
        return sideBarService.getSelectedProducts;
    },

    productSearchAPI: function() {
        let sideBarService = new SideBarService();
        return sideBarService.getSelectedProductsByEmployeeName;
    },
    ...
    productDataHandler: function () {
        let templateObject = Template.instance();
        return function(data) {
            let dataReturn =  templateObject.getProductDataTableList(data)
            return dataReturn
        }
    },

    productExDataHandler: function() {
        let templateObject = Template.instance();
        return function(data) {
            let dataReturn =  templateObject.getProductDataTableList(data)
            return dataReturn
