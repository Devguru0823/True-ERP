...
Template.newtaxratepop.onRendered(async function() {
    ...
    templateObject.getTaxRateDetail = async function() {
        return new Promise((resolve, reject)=>{
            getVS1Data('TTaxcodeVS1').then(function(dataObject) {
                if (dataObject.length === 0) {
                    taxRateService.getTaxRateVS1().then(function(data) {
                        resolve(data.ttaxcodevs1)
                    }).catch(function(err) {
                        resolve([])
                    });
                } else {
    ...
    templateObject.getRecord = async function () {
        let taxRates = await templateObject.getTaxRateDetail();
        if(taxRates && taxRates.length > 0) {
            if(templateObject.data.name) {
                let name = templateObject.data.name;
                let index = taxRates.findIndex(item => {
                    return item.fields.CodeName == name;
                })

                if(index > -1) {
                    let taxRate = (taxRates[index].fields.Rate * 100).toFixed(2) + '%';
                    var dataList = {
                        id: taxRates[index].fields.Id || '',
                        codename: taxRates[index].fields.CodeName || '-',
                        description: taxRates[index].fields.Description || '-',
                        region: taxRates[index].fields.RegionName || '-',
                        rate: taxRate || '-',
    ...
    let records=await templateObject.getRecord();
    templateObject.record.set(records);
